...
# Original Cobalt job ref number
for file in os.listdir():
    if file.endswith(".cobaltlog"):
        cooleyRef = file.split(".")[0]
        break

# Add original job Cobalt ref number to header
restarted = False
restartedIndex = 0
targets = ["restart", "Restart", "restarted", "Restarted"]
...
commentedLines = len([i for i in inpHeader if i.startswith("!")])

if restarted:
    inpHeader[restartedIndex] = "! Restarted from {}\n".format(cooleyRef)
else:
    inpHeader.insert(commentedLines, "! Restarted from {}\n".format(cooleyRef))
